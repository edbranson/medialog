...
async function getAllData() {
    try {
        ...
        await storeEntries(function () {
            document.addEventListener("btn-addEntry.collapse.show", displayEntryUserSelect());
...
//main display of entries for all app functionality through home.html
function displayTable(arg) {
    ...
    for (let i = 0; i < data.length; i++) {
        var row = `<tr id= "entry- + ${data[i].id}" onClick="editEntry(this)" >
                ...
                <td style="display:none" id="entrySubject">${data[i].subject}</td>
                <td style="display:none" id="entryReader">${data[i].reader}</td>
                <td style="display:none" id="entryType">${data[i].type}</td>
                <td style="display:none" id="entryAudience">${data[i].audience}</td>
                <td style="display:none" id="entryPromodesc">${data[i].promoDesc}</td>
                <td style="display:none" id="entrySource">${data[i].source}</td>
                <td id="entryRating">${data[i].rating}</td>
                <td style="display:none" id="entryMycomment">${data[i].myComment}</td>
                <td style="display:none" id="entryRecommended">${data[i].recommendedBy}</td>
                <td style="display:none" id="entryStatus">${data[i].status}</td>
                <td style="display:none" id="entryDatestarted">${data[i].dateStarted}</td>
                <td style="display:none" id="entryDatefinished">${data[i].dateFinished}</td>
                <td align="center">
...
//called from document.eventListener in getAllData()
...
// retrieve the csrf token from cookies ... used in POST headers
...
const addEditEntryForm = document.getElementById("updateEntry");
addEditEntryForm.addEventListener('submit', addEditEntrySubmit);
...
function addEditEntrySubmit(event){
    ...
    let readerInput = inputs['formReader'].value
    let typeInput = inputs['formType'].value
    let audienceInput = inputs['formAudience'].value
    let promodescInput = inputs['formPromodesc'].value
    let sourceInput = inputs['formSource'].value
    let ratingInput = inputs['formRating'].value
    let mycommentInput = inputs['formMycomment'].value
    let recomendedInput = inputs['formRecommended'].value
    let statusInput = inputs['formStatus'].value
    let datestartedInput = inputs['formDatestarted'].value
    let datefinishedInput = inputs['formDatefinished'].value
    ...
    let data = {
       ...
       'promodesc': promodescInput,
       'source': sourceInput,
       'rating': ratingInput,
       'mycomment': mycommentInput,
       'recommended': recomendedInput,
       'status': statusInput,
       'datestarted': datestartedInput,
       'datefinished': datefinishedInput
    };
...
// $("#addEntryuserModal").on('show.bs.modal', displayEntryUserSelect);
//triggered from eventlistener in getAllData() function
function displayEntryUserSelect() {
    data = JSON.parse(sessionStorage.getItem('users'));
    
    // var userModal = getElementById('#addEntryUserModal');
    // userModal.
    var table = document.getElementById("selectUser")
    for (let i = 0; i < data.length; i++) {
        var row = `<tr class="entryUserSelect">     
                <td id="entryUserCheckBox">
                <label for="user-${data[i].id}"><input type="checkbox" id="entryUser-${data[i].id}" name="entryUser" value="${data[i].id}"/>  ${data[i].firstName}</label></td>
                </tr>`       
        table.innerHTML += row
    }
}

function selectedEntryUser(){
    var userChecked = []
    var checkBoxRowsUsers = document.querySelectorAll(`input[name="entryUser"]:checked`);
    for (let i = 0; i < checkBoxRowsUsers.length; i++) { 
        userChecked.push(parseInt(checkBoxRowsUsers[i].value))
    };
    if (userChecked.length === 1) {
        var user = userChecked[0];
        for (let i = 0; i < checkBoxRowsUsers.length; i++) { 
            checkBoxRowsUsers[i].checked = false;
        };
        selectMedia(user)
    }
    
    // userChecked.length === 1 ? var userSelected = userChecked.id) : ()=> console.log('no user'); 
}

function selectMedia(user){
    console.log('hello we are selecting media');
    console.log(user);
}
// const addEntryUserForm = document.getElementById("addEntryUser");
// addEntryUserForm.addEventListener('submit', addEntryUserSubmit);

// function addEntryUserSubmit(event) {
//     event.preventDefault();
...
function editEntry(row) {
    ...
    if (id) {
        ...
        subject = row.cells.namedItem("entrySubject").innerHTML;
        reader = row.cells.namedItem("entryReader").innerHTML;
        type = row.cells.namedItem("entryType").innerHTML;
        audience = row.cells.namedItem("entryAudience").innerHTML;
        promodesc = row.cells.namedItem("entryPromodesc").innerHTML;
        source = row.cells.namedItem("entrySource").innerHTML;
        rating = row.cells.namedItem("entryRating").innerHTML;
        mycomment = row.cells.namedItem("entryMycomment").innerHTML;
        recommended = row.cells.namedItem("entryRecommended").innerHTML;
        status = row.cells.namedItem("entryStatus").innerHTML;
        datestarted = row.cells.namedItem("entryDatestarted").innerHTML;
        datefinished= row.cells.namedItem("entryDatefinished").innerHTML;

        //load values from record to be edited
        $('#form-id').val(id);
        $('#form-user').val(user);
        ...
        $('#form-reader').val(reader);
        $('#form-type').val(type);
        $('#form-audience').val(audience);
        $('#form-promodesc').val(promodesc);
        $('#form-source').val(source);
        $('#form-rating').val(rating);
        $('#form-mycomment').val(mycomment);
        $('#form-recommended').val(recommended);
        $('#form-status').val(status);
        $('#form-datestarted').val(datestarted);
        $('#form-datefinished').val(datefinished);
    }
}
...
// function addEntry() {
//     let isUser = document.get elementById("updateEntry").elements['formUser']
//     if 
// }
...
document.querySelector('#selectUser').addEventListener('click', selectedEntryUser, false )
...
// document.querySelector('#UpdateEntry').addEventListener('click', "", false);
